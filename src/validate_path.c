//flood fill is used for checking if all Cs and Es are reachable, we flood fill the array and then we check if there are Cs or Es left
//here we dont collect the Cs or we dont exit, we just flood fill the array to check if the game is winnable
...
void find_player(char **map, int *x, int *y)           //Players position
{
	int	i;
	int	j;
